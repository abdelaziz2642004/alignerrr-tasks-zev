...
from zev.workflow_variables import (
    display_variables_summary,
    extract_variables_from_workflow,
    has_variables,
...
def handle_workflow(workflow: Workflow, original_query: str = ""):
    ...
    # Check for variables and prompt if needed
    variables: dict[str, str] = {}
    if has_variables(workflow):
        var_definitions = extract_variables_from_workflow(workflow)
        variables = prompt_for_variables(var_definitions)
        if variables is None:  # User cancelled
            rprint("[yellow]Cancelled.[/yellow]")
            return
        if variables:
            display_variables_summary(variables)
    ...
    if action == "run_all":
        run_workflow_all(workflow, original_query, variables=variables)
    elif action == "copy_all":
        copy_workflow_commands(workflow, variables=variables)
    elif action == "interactive":
        run_workflow_interactive(workflow, original_query, variables=variables)
...
def run_workflow_all(workflow: Workflow, original_query: str = "", state: Optional[WorkflowState] = None, start_from_step: int = 1, variables: dict[str, str] = None):
    ...
    if state is None:
        ...
        # Use variables from state when resuming, but allow overrides
    ...
    for step in workflow.steps:
        ...
        actual_command = substitute_variables(step.command, variables)
...
def copy_workflow_commands(workflow: Workflow, variables: dict[str, str] = None):
    variables = variables or {}
    # Join all commands with newlines for easy pasting (with variables substituted)
    all_commands = "\n".join(substitute_variables(step.command, variables) for step in workflow.steps)
    try:
...
def run_workflow_interactive(workflow: Workflow, original_query: str = "", state: Optional[WorkflowState] = None, start_from_step: int = 1, variables: dict[str, str] = None):
    manager = _get_workflow_state_manager()
    variables = variables or {}
...
def resume_workflow(state: WorkflowState):
    ...
    if state.variables:
        display_variables_summary(state.variables)
    ...
    if state.variables:
        choices.append(questionary.Choice("Edit variables before resuming", value="edit_vars"))
    choices.extend([
    ...
    if action == "cancel":
        ...
        return
    elif action == "edit_vars":
        # Re-prompt for variables with current values as defaults
        var_definitions = extract_variables_from_workflow(state.workflow)
        new_variables = prompt_for_variables(var_definitions, state.variables)
        ...
        manager = _get_workflow_state_manager()
        manager.save_state(state)
        # Recursively call resume to let user choose execution mode
